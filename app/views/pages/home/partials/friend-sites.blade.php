<div class="friend-sites">
	<hr>
	<div class="toggle-button center-block" id="friend-sites-toggle-button">
		<span>Sitios de interés</span>

		<div class="external-circle center-block">
			<span class="icon icon-arrow-friends-sites icon-circle" id="friend-sites-button"></span>
		</div>
	</div>


	<div class="friends-sites-pane collapse">
		<div class="row ">
			<div class="col-md-12">
				<div class="col-md-4">
					<h5> Sitios UNAM </h5>
						<ul class="footer_links list-unstyled">
							<li>
								<a class="footer_link" href="http://www.cultura.unam.mx/">
									Cultura UNAM
								</a>
							</li>
							<li>
								<a class="footer_link" href="http://www.cuec.unam.mx/">
									Centro Universitario de Estudios Cinematográficos
								</a>
							</li>
							<li>
								<a class="footer_link" href="http://www.chopo.unam.mx/cartelera_cine.html">
									Museo Universitario del Chopo – Cine
								</a>
							</li>
							<li>
								<a class="footer_link" href="http://www.unamenlinea.unam.mx/">
									Toda la UNAM en línea
								</a>
							</li>
							<li>
								<a class="footer_link" href="http://www.libros.unam.mx/">
									Libros UNAM
								</a>
							</li>
							<li>
								<a class="footer_link" href="http://www.casadellago.unam.mx/sitio/index.php/actividades/cine">
									Casa del Lago - Cine
								</a>
							</li>
						</ul>
					</div>
					<div class="col-md-4">
						<h5> Festivales de cine </h5>
						<ul class="footer_links list-unstyled">
							<li>
								<a class="footer_link" href="http://ambulante.com.mx/es">
								Ambulante Gira de Documentales
								</a>
							</li>
							<li>
								<a class="footer_link" href="http://distrital.mx/">	
									Distrital. Cine y otros mundos</a>
							</li>
							<li>
								<a class="footer_link" href="http://www.docsdf.org/">
									DOCSDF Festival Internacional de Cine Documental de la Ciudad de México
								</a>
							</li>
							<li>
								<a class="footer_link" href="http://www.ficunam.unam.mx/">
									Festival Internacional de Cine de la UNAM
								</a>
							</li>
							<li>
								<a class="footer_link" href="http://www.ficaacapulco.com">
									Festival Internacional de Cine de Acapulco
								</a>
							</li>
							<li>
								<a class="footer_link" href="http://www.ficags.com.mx/">
									Festival Internacional de Cine de Aguascalientes
								</a>
							</li>
							<li>
								<a class="footer_link" href="http://www.dhfest.org/">
									Festival Internacional de Cine de Derechos Humanos
								</a>
							</li>
							<li>
								<a class="footer_link" href="https://www.ficg.mx/29/index.php/es/">
									Festival Internacional de Cine de Guadalajara
								</a>
							</li>
							<li>
								<a class="footer_link" href="http://www.monterreyfilmfestival.com/">
									Festival Internacional de Cine de Monterrey
								</a>
							</li>
							<li>
								<a class="footer_link" href="http://moreliafilmfest.com/">
									Festival Internacional de Cine de Morelia
								</a>
							</li>
							<li>
								<a class="footer_link" href="http://www.expresionencorto.com/">
									Festival Internacional de Cine Expresión en Corto
								</a>
							</li>
							<li>
								<a class="footer_link" href="	http://www.macabro.mx/">
									Macabro: Festival Internacional de Cine de Horror de la Ciudad de México
								</a>
							</li>
							<li>
								<a class="footer_link" href="	http://www.lamatatena.org/es/noticias.html">
									Festival Internacional de Cine para Niños (...y no tan niños)
								</a>
							</li>
							<li>
								<a class="footer_link" href="	http://www.mujerescineytv.org/">
									Mujeres en el cine y la televisión 
								</a>
							</li>
							<li>
								<a class="footer_link" href="	http://shortshortsfilmfestivalmexico.com/">
									Short Short Film Festival México
								</a>
							</li>
						</ul>
					</div>
					<div class="col-md-4">
						<h5> Instituciones culturales </h5>
						<ul class="list-unstyled">
							<li><a class="footer_link" href="http://www.cultura.unam.mx/">Cultura UNAM</a></li>
							<li><a class="footer_link" href="http://www.cuec.unam.mx/">Centro Universitario de Estudios Cinematográficos</a></li>
							<li><a class="footer_link" href="http://www.chopo.unam.mx/cartelera_cine.html">Museo Universitario del Chopo – Cine</a></li>
						</ul>
				</div>
			</div>		
		</div>
	</div>
</div>