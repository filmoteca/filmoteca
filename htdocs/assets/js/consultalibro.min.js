...
$(document).ready(function () {
    ...
    var MIN_LETTER_INDEX = 0;
    var MAX_LETTER_INDEX = 1;
    ...
    var filterBooks = function (event, ui) {
        ...
        $books.each(function (index, book) {
            var $book = $(book);
            var titleFirst = parseInt($book.data('title-first'));

            var mineIs = ( alphabet[ui.value] === mine ) ? "Mine too!! 😂✌" : "But mine is a " + mine + "! 😞",
                yoursIs = "Oh golly gosh, your favourite animal is a " + alphabet[ui.value] + "? — " + mineIs;
            

            //if (titleFirst >= ui.values[MIN_LETTER_INDEX] && titleFirst <= ui.values[MAX_LETTER_INDEX]) {
              //  $book.show();
                //book++;
            //} else {
              //  $book.hide();
    ...
    $letterSelector.on('slidecreate', function () {
        filterBooks({}, {values: filmotecaConfig.consultaLibro.slider.value});
    });
    ...
    $( '#book-searcher' ).autocomplete({
        ...
        return $('<li>')
            .append(
                '<div><img src="' +item.url_image + '">' + item.title + '<br>' + item.award_date + '</div>'
            )
            .appendTo(ul);
    };
});