	...
	var methods = {
		init: function(settings) {
			...
			return $(this).each(function() {
				...
				ops.totalTime = ops.timeTransition * (ops.numStrips + 1);

				ops.imgs = $this.find('li img');
				ops.infos = $this.find('li .info');
				...
				//obtenemos el porcentaje del elemento.
				var parentWidth = $(ops.imgs).offsetParent().width();
				var imageWidth = 100 * $(ops.imgs[0]).width() / parentWidth;

				ops.opsCssStrips = {
					marginTop: '-200px',
					width: ( imageWidth / ops.numStrips) + '%',
					opacity: 0,
					float: 'left',
					overflow: 'hidden'
				};
		...
		goto: function(newIndex) {
			...
			ops.infos.each(function(){
				$(this).hide()
			});

			$(ops.infos[newIndex]).delay(ops.totalTime).fadeIn(300);
